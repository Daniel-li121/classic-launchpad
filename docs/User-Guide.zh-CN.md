...
本手册适用于 Classic Launchpad 1.0.5，仅支持运行 macOS 15 或更高版本的 Apple 芯片 Mac，不再支持 Intel Mac。界面会根据 macOS 首选语言自动切换简体中文、English 或日本語，其他语言默认使用英文。
...
Classic Launchpad 作为登录项启动时，macOS 可能仍在初始化触控板服务。现在 App 会验证监听器是否真正安装成功，并自动持续重试，直到触控板可用；重启 Mac 后不再需要退出并重新打开 App。稍后接入外置触控板时，同一套重试机制也会自动连接。
